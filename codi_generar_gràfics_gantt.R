...
tratamiento_SAP_nombres <- tratamiento_SAP %>% select( # Vigilar format dates!
  nhc = `Numero de historia clinica`,
  numero_episodio = FALNR,
  tratamiento = MOTX,
  via_administracion = APROU,
  fecha_inicio = MOVDF,
  fecha_final = MOVDT
)

# Carregar el formulari actual del Redcap.
formulari_redcap <- import("documents_necessaris/Tratamiento_Redcap.csv")
...
codificacio_tractaments <- import("documents_necessaris/Codificación_tratamiento.csv")
...
### Selecció dels tractaments d'interès i modificació del nom del tractament.
...
# Modificar la columna tractament eliminant tot el que vingui després d'una coma o d'un número.
tratamiento_SAP_atb_atv_atf_abreviado <- tratamiento_SAP_atb_atv_atf
tratamiento_SAP_atb_atv_atf_abreviado$tratamiento <- gsub(",.*| \\d+.*", "", tratamiento_SAP_atb_atv_atf_abreviado$tratamiento)
...
tratamiento_SAP_atb_atv_atf_abreviado_clas <- tratamiento_SAP_atb_atv_atf_abreviado %>%
  left_join(classificacio_tractaments %>% 
              select(tratamiento, `tipo de tratamiento`), by = c("tratamiento" = "tratamiento"))

# Codifiquem la variable tractament amb els codis del Redcap.
# Primer agafem un vector amb els noms dels tractaments amb estructura actuals, un vector de la seva traducció amb les labels del Redcap per comprovar que són coincidents i finalment un vector amb la codificació numèrica Redcap. 
...
# Comprovem com queda amb les labels. Ens interessa aquesta perquè estem creant gràfics.
tratamiento_nombres_recod <- tratamiento_SAP_atb_atv_atf_abreviado_clas
tratamiento_nombres_recod$tratamiento <- factor(tratamiento_nombres_recod$tratamiento, levels=nombres_tratamiento_actual, labels=nombres_tratamiento_redcap)

# Recodifiquem la via_administracion per tenir valors en format Redcap. Creem nou df per emmagatzemar dades.
tratamiento_SAP_recod <- tratamiento_nombres_recod
# Creem nova columna amb els noms recodificats de la via d'administració.
tratamiento_SAP_recod$via_administracion_recod <- codificacio_adm$DESCR[match(tratamiento_SAP_recod$via_administracion, codificacio_adm$APROUID)]

# Ara recodifiquem la via_administracion creant una nova columna que recodifica les vies d'administració en format Redcap.
tratamiento_via_recod <- tratamiento_SAP_recod
tratamiento_via_recod$via_administracion_redcap <- codificacio_adm$REDCAP[match(tratamiento_via_recod$via_administracion_recod, codificacio_adm$DESCR)]
...
# S'ha detectat que hi ha algun tractament sense via d'administració i aquest és sempre la tobramicina neb, que s'entén que la via d'administració és nebulització, per tant: 1.
tratamiento_via_tob_recod <- tratamiento_via_recod %>% 
  mutate(via_administracion_redcap = if_else(tratamiento == "Tobramicina", 
                                             "Nebulizado", # Cofificació nebulització.
                                             via_administracion_redcap)) # Mantenir el valor tal qual.

# És important destacar que he considerat una via d'administració que era "Bucal" com a "Oral" per la Nistatina per evitar afegir texto libre.
# Ara afegim una nova columna que en cas que sigui "Otra" (== 4) s'especifiqui la via d'administració en format "texto libre".
tratamiento_via_otra_recod <- tratamiento_via_tob_recod %>% 
  mutate(especif_via_administracion_recod = ifelse(via_administracion_redcap == "Otra", via_administracion_recod, NA)) %>% 
  select(-via_administracion_recod, -via_administracion)
...
# Verificar que totes les dates tinguin el mateix format. IMPORTANT: En el document csv les variables han d'estar en format data (%y-%m-%d), si no no funciona!
tratamiento_via_otra_recod$fecha_inicio <- as.Date(tratamiento_via_otra_recod$fecha_inicio)
tratamiento_via_otra_recod$fecha_final <- as.Date(tratamiento_via_otra_recod$fecha_final)
formulari_redcap$ant_ingreso_fecha_hosp <- as.Date(formulari_redcap$ant_ingreso_fecha_hosp)
formulari_redcap$out_alta_hosp_fecha <- as.Date(formulari_redcap$out_alta_hosp_fecha)

# Selecció de tractaments que es troben dins del ingrés hospitalari del pacient.
tratamiento_SAP_episodio <- tratamiento_via_otra_recod %>%
  left_join(formulari_redcap %>% select(nhc, ant_ingreso_fecha_hosp, out_alta_hosp_fecha), by = c("nhc"="nhc"), relationship = "many-to-many") %>% 
  filter((fecha_inicio >= ant_ingreso_fecha_hosp &
            fecha_inicio <= out_alta_hosp_fecha) &
           (fecha_final >= ant_ingreso_fecha_hosp & 
              fecha_final <= out_alta_hosp_fecha)) %>% 
...
# Ara creem una funció per ajuntar periodes superposats, considerant els dies adjacents com a superposicions també.
merge_overlaps <- function(df) {
  # Creem un df per emmagatzemar els resultats.
  results <- data.frame()
  while(nrow(df) > 0) {
    # Agafem la primera línia.
    actual <- df[1, ]
    df <- df[-1, ]
    # Trobem superposicions, incloent aquelles que comencen el dia després de actual$fecha_final
    overlaps <- which(df$fecha_inicio <= actual$fecha_final + 1)
    while(length(overlaps) > 0) {
      # Extenem el periode actual fins la última fecha_final entre les superposicions.
      actual$fecha_final <- max(actual$fecha_final, max(df[overlaps,]$fecha_final))
      # Eliminem les files superposades del df.
      df <- df[-overlaps, ]
      # Busquem noves superposicions després d'actualitzar el periode actual.
      overlaps <- which(df$fecha_inicio <= actual$fecha_final + 1)
    }
    # Afegim el periode actual als resultats
    results <- rbind(results, actual)
  }
  results
}

# Apliquem aquesta funció al dataframe
tratamiento_SAP_periodos <- tratamiento_SAP_unico %>%
  ...
  select(nhc, tratamiento, via_administracion_redcap, especif_via_administracion_recod, `tipo de tratamiento`, fecha_inicio, fecha_final)

# Eliminem els tractaments que són bolus i que no tenen continuitat en el temps.
# Seleccionem aquests tractaments primer de tot i els guardem en un df.
tratamiento_bolus_aislado <- tratamiento_SAP_periodos %>%
  group_by(nhc, tratamiento) %>%
  mutate(duracion_dias = as.numeric(difftime(fecha_final, fecha_inicio, units = "days"))) %>%
...
tratamiento_SAP_no_bolus <- anti_join(tratamiento_SAP_periodos, tratamiento_bolus_aislado, 
                                      by = c("nhc", "tratamiento", "via_administracion_redcap",
                                             "especif_via_administracion_recod", "tipo de tratamiento",
                                             "fecha_inicio", "fecha_final"))

# Modificar els noms de les columnes perquè siguin iguals que les del Redcap. En aquest cas no té massa sentit però ho deixem així posat per no modificar-ho tot.
tratamiento_nombres_columna <- tratamiento_SAP_periodos %>% 
  rename(t_trat = tratamiento, t_trat_tipo = `tipo de tratamiento`, t_trat_adm = via_administracion_redcap, t_trat_adm_otro = especif_via_administracion_recod, t_trat_fecha_i = fecha_inicio, t_trat_fecha_f = fecha_final) %>% 
...
tractament_gantt <- tratamiento_nombres_columna %>%
  left_join(formulari_redcap %>% select(nhc, ant_id), join_by(nhc)) %>% 
